...
- 자판기가 보유하고 있는 금액으로 동전을 무작위로 생성한다.
  - 투입 금액으로는 동전을 생성하지 않는다.
  - → 보유 금액 내에서 해결
- 잔돈 반환 시, 보유한 최소 개수의 동전으로 잔돈을 반환
- 지폐는 없다고 가정
- 상품명, 가격, 수량을 입력해 상품을 추가
  - 상품은 최소 100원, 10원 단위
- 사용자가 투입한 금액으로 상품을 구매한다.
- (투입하고 사용해서 → 서비스 돌아가면서)남은 금액이 상품의 최저 가격보다 적거나, 모든 상품이 소진된 경우 바로 잔돈을 반환한다.(더 이상 로직을 실행하지 않는다.)
- 잔돈을 반환할 수 없는 경우 가능한 금액만 반환
  - 반환 되지 않은 금액은 자판기에 남는다.
- 에러 발생시 다시 입력 받기
...
- 자판기 보유 금액에서 실행
- 500,100,50,10 순서로 진행
- 총 금액이 0이 될 때까지 랜덤 개수 생성해서 저장
...
- 남은 투입 금액과 자판기 반환 가능 여부 비교
- 반환 가능한 금액 내에서 최소 값으로 반환
